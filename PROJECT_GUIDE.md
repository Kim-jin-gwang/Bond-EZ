# ⚙️ [프로젝트 가이드] 사용 방법 및 구조 정의서 - 채권이지(BondEZ)

이 문서는 **채권이지(BondEZ)** 서비스의 시스템 설계 구조, HDFS 데이터 파티셔닝 정책, 복구 및 실행 절차, 데이터 정합성 검증 SQL, REST API 규격서 및 시스템 포트 매핑 정보를 상세히 정리한 운영 가이드라인입니다.

---

## 📌 목차 바로가기 (Quick Navigation)
*   [🎈 1. 서비스 소개: 채권이지(BondEZ)](#1-서비스-소개-채권이지bondez)
*   [🏗️ 2. 계층별 데이터 아키텍처 (Data Architecture Layers)](#2-계층별-데이터-아키텍처-data-architecture-layers)
*   [📂 3. 프로젝트 디렉토리 및 HDFS 구조 (Directory & HDFS Structure)](#3-프로젝트-디렉토리-및-hdfs-구조-directory--hdfs-structure)
*   [🛠️ 4. 관계형 DB 테이블 명세 및 ERD](#4-관계형-db-테이블-명세-및-erd)
*   [🚦 5. 신규 환경 복구 및 최초 실행 절차 (Setup & Run Guide)](#5-신규-환경-복구-및-최초-실행-절차-setup--run-guide)
*   [🌐 6. REST API 명세 (API Specifications)](#6-rest-api-명세-api-specifications)
*   [🌐 7. 서비스 포트 및 웹 콘솔 정보](#7-서비스-포트-및-웹-콘솔-정보)

---

## 1. 서비스 소개: 채권이지(BondEZ)

**채권이지(BondEZ)**는 개인 투자자들에게 정보 접근성을 높이기 위해 채권 데이터(스펙, 이자, 옵션)와 시장 시세, 실시간 관련 뉴스, 경제 지표 및 금융 용어를 한곳에 통합 제공하는 **스마트 채권 포털** 서비스입니다.

---

## 2. 계층별 데이터 아키텍처 (Data Architecture Layers)
...
## 3. 프로젝트 디렉토리 및 HDFS 구조 (Directory & HDFS Structure)
...
## 5. 신규 환경 복구 및 최초 실행 절차 (Setup & Run Guide)

신규 로컬 환경에서 전체 서비스를 빌드하고 데이터 수집까지 완료하는 단계별 절차입니다.

### [Step 1] 사전 요구사항 확인 및 Docker 구동
1. 작업 컴퓨터에 **Docker Desktop**이 설치되어 실행 중인지 확인합니다.
2. 터미널(Git Bash, WSL, Terminal 등)을 프로젝트 루트 디렉토리(`de_pjt/`)에서 엽니다.
...
cp .env.example .env
```
2. `.env` 파일을 텍스트 에디터로 열어 필요한 변수를 기입합니다.
   * `DATA_PORTAL_API_KEY`: 공공데이터포털 서비스 인증키
...
sh service.sh up
```
* **동작**: 이미지 빌드 ➔ Elasticsearch/Kibana 기동 ➔ Django 백엔드가 실행될 때까지 대기 ➔ 데이터베이스 마이그레이션 적용 및 서비스 개시.
* **소요 시간**: 약 2~3분 소요 (최초 빌드 시).

### [Step 4] 데이터 파이프라인 구동 및 기초 데이터 로드
```bash
sh data.sh up
```
* **동작**: 공통 Kafka, HDFS, Spark, Flink 및 Airflow 기동 ➔ HDFS 파일 시스템 디렉토리 생성 및 초기화 ➔ 경제 용어 데이터베이스(`glossary`) 자동 적재.

### [Step 5] Airflow 스케줄러를 이용한 최초 데이터 적재
1. 웹 브라우저를 열고 Airflow 관리자 콘솔 주소인 [http://localhost:8081](http://localhost:8081)에 접속합니다.
2. 로그인 창에 기본 계정(`admin` / `admin`)을 입력하여 접속합니다.
3. DAG 목록 화면에서 **`bond_full_pipeline_dag`**를 활성화(Active)로 토글합니다.
4. 우측 플레이 버튼(▷) ➔ **`Trigger DAG`**를 클릭하여 파이프라인을 작동시킵니다.
5. **검증**: 약 2~3분 뒤 작업이 모두 초록색(`Success`)으로 완료되면 데이터베이스 적재가 성공한 것입니다.
...
* **Vue 웹 화면**: [http://localhost:5173](http://localhost:5173) (메인 대시보드 및 채권 시세 탭 로드)
* **Kibana UI**: [http://localhost:5601](http://localhost:5601) (Elasticsearch 인덱싱 상태 확인)

---

## 6. REST API 명세 (API Specifications)

모든 API 호출은 공통 프리픽스로 `/api/v1`을 가집니다.
...
* `GET /bonds/filter-options` : 검색 필터 바인딩용 메타 목록 조회
