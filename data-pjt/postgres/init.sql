...
-- 5-1. ERD에 명시된 NewsProvider 테이블 생성
CREATE TABLE IF NOT EXISTS "NewsProvider" (
...
CREATE TABLE IF NOT EXISTS "News" (
    ...
    source_id BIGINT NOT NULL REFERENCES "NewsProvider"(provider_id) ON DELETE CASCADE,
...
-- 5-3. ERD에 명시된 GlossaryCategory 테이블 생성
...
CREATE TABLE IF NOT EXISTS "Glossary" (
    ...
    category_id BIGINT NOT NULL REFERENCES "GlossaryCategory"(category_id) ON DELETE CASCADE,
...
BEGIN
    -- NewsProvider에 언론사 정보 삽입/가져오기 (최대 50자 제한)
    INSERT INTO "NewsProvider" (provider_name, created_at, updated_at)
...
-- 7-5. BondType (채권 종류) 테이블 생성
CREATE TABLE IF NOT EXISTS "BondType" (
...
-- 7-8. GuaranteeStatus (보증 여부) 테이블 생성
CREATE TABLE IF NOT EXISTS "GuaranteeStatus" (
...
-- 7-9. BondOptionExercise (옵션 행사 가능일 상세) 테이블 생성
CREATE TABLE IF NOT EXISTS "BondOptionExercise" (
...
-- 7-10. BondCashflowRule (이자 지급 상세 조건) 테이블 생성
CREATE TABLE IF NOT EXISTS "BondCashflowRule" (
...
CREATE TABLE IF NOT EXISTS "Bond" (
    ...
    bond_type_id BIGINT NOT NULL REFERENCES "BondType"(bond_type_id),
    ...
    option_type option_type_enum NOT NULL,
    cashflow_rule_id BIGINT NULL REFERENCES "BondCashflowRule"(cashflow_rule_id) ON DELETE SET NULL,
    ...
    option_exercise_id BIGINT NOT NULL REFERENCES "BondOptionExercise"(option_exercise_id),
    guarantee_status_id BIGINT NOT NULL REFERENCES "GuaranteeStatus"(guarantee_status_id),
    rating_id BIGINT NOT NULL REFERENCES "CreditRating"(rating_id),
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    deleted_at TIMESTAMP NULL
);
...
-- 7-13. UserBond (사용자 채권 구매 정보) 테이블 생성
CREATE TABLE IF NOT EXISTS "UserBond" (
...
-- 7-14. BondMarketData (채권 시장정보) 테이블 생성
CREATE TABLE IF NOT EXISTS "BondMarketData" (
...
-- 8-1. BondType 시드 데이터
INSERT INTO "BondType" (bond_type) VALUES
...
-- 8-3. GuaranteeStatus 시드 데이터
INSERT INTO "GuaranteeStatus" (guarantee_status) VALUES
('무보증'),
('보증')
ON CONFLICT (guarantee_status) DO NOTHING;

-- 8-4. CreditRating 시드 데이터
INSERT INTO "CreditRating" (rating_name, rating_order) VALUES
...
BEGIN
    ...
    FOR r IN SELECT * FROM temp_bonds_master_staging LOOP
        ...
        -- 채권 분류 ID 조회 (기본값 설정)
        SELECT bond_type_id INTO v_bond_type_id FROM "BondType" WHERE bond_type = r.bond_type;
        IF v_bond_type_id IS NULL THEN
            SELECT bond_type_id INTO v_bond_type_id FROM "BondType" WHERE bond_type = '일반회사채';
        END IF;
        ...
        -- 보증 여부 ID 조회
        SELECT guarantee_status_id INTO v_guarantee_status_id FROM "GuaranteeStatus" WHERE guarantee_status = r.guarantee_status;
        IF v_guarantee_status_id IS NULL THEN
            SELECT guarantee_status_id INTO v_guarantee_status_id FROM "GuaranteeStatus" WHERE guarantee_status = '무보증';
        END IF;

        -- 신용등급 ID 조회
        SELECT rating_id INTO v_rating_id FROM "CreditRating" WHERE rating_name = r.credit_rating;
        IF v_rating_id IS NULL THEN
            SELECT rating_id INTO v_rating_id FROM "CreditRating" WHERE rating_name = 'BBB-';
        END IF;
        ...
        IF v_option_exercise_id IS NULL THEN
            INSERT INTO "BondOptionExercise" (option_type) VALUES (v_option_type) RETURNING option_exercise_id INTO v_option_exercise_id;
        ELSE
            UPDATE "BondOptionExercise" SET option_type = v_option_type WHERE option_exercise_id = v_option_exercise_id;
        END IF;

        IF v_cashflow_rule_id IS NULL THEN
            INSERT INTO "BondCashflowRule" (interest_payment_method) VALUES (r.interest_type) RETURNING cashflow_rule_id INTO v_cashflow_rule_id;
        END IF;
